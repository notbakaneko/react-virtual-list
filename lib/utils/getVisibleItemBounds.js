...
var getVisibleItemBounds = function getVisibleItemBounds(list, container, items, itemHeight, itemBuffer) {
  var columns = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1;
  ...
  var firstItemIndex = Math.max(0, (Math.floor(listViewTop / itemHeight) - itemBuffer) * columns);
  var lastItemIndex = Math.min(items.length, (Math.ceil(listViewBottom / itemHeight) + itemBuffer) * columns) - 1;
